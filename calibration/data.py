...
class VideoDataset(D.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if test_fname in self.Q_per_ch:
            ...
            batches = len(features['t0_b0'])
            frames = len(features['t0_b0'][0])
            ...
            resampled_qpc = np.empty( (batches, temp_channels,frames,resampled_bands), dtype=np.float32)
            qpc = np.empty( (batches, temp_channels,frames,bands), dtype=np.float32)

            for batch in range(batches):
                for bb in range(bands):
                    for cc in range(temp_channels):
                        qpc[batch,cc,:,bb] = np.array( features[f't{cc}_b{bb}'][batch] ).reshape( (1, 1, 1,frames) )

                if self.resample:
                    for tt in range(frames):
                        lut = interp1d(rho_band, np.insert(qpc[batch, cc,tt], 0, 0) if extrapolate else qpc[batch, cc,tt])
                        resampled_qpc[batch, cc,tt] = np.append(lut(2**np.linspace(self.log_rho_max, self.log_rho_min, resampled_bands-1)), qpc[batch, cc,tt,-1])
