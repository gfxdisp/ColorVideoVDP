...
def main():
    ...
    if args.masking == 'base' and args.pooling == 'base':
        metric = pycvvdp.cvvdp(quiet=True, device=device, temp_padding='replicate', config_paths=args.config_paths)
        # params = [metric.ch_weights, metric.baseband_weight, metric.beta_sch, metric.beta_tch, metric.beta_t]     # betas are int
        params = [metric.ch_chrom_w, metric.ch_trans_w, metric.baseband_weight]
    else:
    ...
    logging.info( f'New ColorVideoVDP parameters saved in "{output_file}"' )
