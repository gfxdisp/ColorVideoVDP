# This is a sample code for blurred optimization 
# Stack batches with pre-defined sizes using schedulers, and anneal the noise with iterations 
# WARNING: This code is not optimized for performance and may take a long time to run.
...
num_iters = 1500
noise_scale = 1.0
loss_type = 'cvvdp' ## Loss type can be different 
lr = 0.01
WRITE_FREQ = 10
...
output_path = f'examples/cvvdp_ML_optimization_noise{noise_scale}_iter{num_iters}_lossCVVDPSimulatedAnnealing{simulated_annealing_schedule[0]}to{simulated_annealing_schedule[-1]}_lr{lr}timesbatch_batch{batch_size_schedule[0]}to{batch_size_schedule[-1]}'
...
if device.type == "cpu":
    ...
    # (Dongyeon) CVVDP is non-deterministic by default --> but this increases stochasticity of the optimization by somehow
...
cvvdp = pycvvdp.cvvdp(display_name='standard_test_4k') # Pass additional parameters as 'standard_test_4k' initially has contrast of 1000000, removes the elevated black level.
...
for kk in range(num_iters):
    ...
    if kk % WRITE_FREQ == 0:
        with torch.no_grad():
            ...
            # Save the frame as an image file in the output_path directory
            imageio.imwrite(os.path.join(output_path, f"frame_{kk:04d}.png"), frame)

            # frames.append(frame)
            plt.close(fig)

#             # ---------------- End of Visualization Code ----------------
# # This does not work with the updated version of tifffile, so check the version and downgrade if necessary
